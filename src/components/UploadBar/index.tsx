    ...
    <div className="bg-white border-t border-slate-200 p-[13px_18px] flex items-center justify-between shadow-[0_-4px_16px_rgba(0,0,0,0.05)] sticky bottom-0 z-10">
      ...
      <div className="flex items-center gap-[8px]">
        <button
          onClick={onOverride}
          disabled={!result}
          className={`inline-flex items-center gap-[6px] px-[16px] py-[8px] rounded-lg bg-transparent text-amber-500 border-[1.5px] border-amber-500 text-[13px] font-semibold transition-colors ${result ? 'cursor-pointer opacity-100' : 'cursor-not-allowed opacity-45'}`}
        >
          Override &amp; Upload
        </button>
        ...
        <button
          ...
          className={`inline-flex items-center gap-[6px] px-[16px] py-[8px] rounded-lg border-0 text-[13px] font-semibold transition-colors ${
            canUpload && result
