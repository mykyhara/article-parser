...
function ArticleBody({ html }: { html: string }) {
  ...
  return (
    <div
      ref={ref}
      className="article-body min-h-[200px] overflow-y-auto pb-[24px]"
...
export function ArticlePreviewPanel({ articleHtml }: ArticlePreviewPanelProps) {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-[12px] p-[18px_18px_0] mb-[10px] shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
      <div className="flex items-center justify-between mb-[14px]">
        <div className="text-[11px] font-semibold text-slate-400 tracking-[0.06em] uppercase">
          Article Preview
        </div>
        <div className="flex bg-slate-50 rounded-[8px] p-[3px] border border-slate-200 gap-[2px]">
          {(['preview', 'html'] as const).map((key) => (
            <button
              key={key}
              onClick={() => setTab(key)}
              className={`px-[12px] py-[4px] rounded-[6px] border-none cursor-pointer text-[12px] font-medium transition-all duration-[120ms] ${
                tab === key
      ...
      {tab === 'preview' ? (
        ...
        <div className="relative">
          <button
            onClick={handleCopy}
            className="absolute top-[10px] right-[10px] z-[2] flex items-center gap-[5px] px-[10px] py-[5px] rounded-[6px] text-[11px] font-medium cursor-pointer bg-slate-800 text-slate-400 border border-slate-700 transition-all duration-150"
          >
            <svg
              width="12"
              height="12"
          ...
          <pre className="max-h-[520px] overflow-y-auto m-0 p-[14px_14px_18px] bg-slate-900 text-slate-400 rounded-[0_0_10px_10px] text-[12px] leading-[1.65] font-code whitespace-pre-wrap break-all">
