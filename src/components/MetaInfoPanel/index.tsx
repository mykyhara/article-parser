...
function CharBadge({ count, limit }: { count: number; limit: number }) {
  ...
  return (
    <span
      className={`inline-flex items-center px-[6px] py-[2px] text-[11px] font-medium rounded-[6px] font-mono border ${colorCls}`}
    >
      {count}/{limit}
    </span>
  )
...
export function MetaInfoPanel({
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-[12px] p-[18px_18px_4px] mb-[10px] shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
      ...
      {FIELDS.map(({ key, label, limit, multi }) => {
        ...
        const inputCls = `w-full text-[13px] text-slate-900 rounded-[8px] p-[8px_12px] bg-white outline-none transition-[border-color,box-shadow] duration-150 ${
          isFocused
            ? 'border border-blue-500 shadow-[0_0_0_3px_rgba(59,130,246,0.1)]'
